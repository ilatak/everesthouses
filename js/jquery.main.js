...
// class on click init
function initClassOnClick() {
	var activeClass = 'active';
	var links = jQuery('#nav a');
	
	links.bind('click', function(e) {
		links.parents().removeClass(activeClass);
		$(this).parent().addClass(activeClass);
		
		if(pageLayout.iScrollLayout) {
			e.preventDefault();
			pageLayout.iScrollLayout.scrollToElement($(this).attr('href'));
		}
        else {
...
function initCurrentSection() {
    $(window).bind('scroll resize', function() {
        ...
        $('.container').each(function(){
            ...
            if($(window).scrollTop() >= $('#'+element).offset().top - 100)
            {
                currentSection = element;
            }
        });

        $('#nav li').removeClass('active').find('a[href="#'+currentSection+'"]').parent().addClass('active');
    });
}
